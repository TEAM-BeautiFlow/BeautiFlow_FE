...
import { ChevronLeft, X, Plus, Clock, ChevronRight } from 'lucide-react'; // 필요한 아이콘 임포트
...
const AppointmentBooking = () => {
  ...
  // 요청사항 텍스트 상태
  const [description, setDescription] = useState('최대한 빨리 시술 부탁드립니다.'); // Figma에 있는 초기 텍스트 설정
  ...
  return (
    <div className="max-w-sm mx-auto min-h-screen" style={{ 
      ...
      <div style={{ padding: '0 20px 32px' }}> {/* Figma에 맞춰 패딩 조정 */}
        <h2 className="label1" style={{ color: 'var(--color-white)', marginBottom: '16px' }}>시술 정보</h2> {/* Figma에 맞춰 폰트 및 마진 조정 */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', paddingBottom: '24px', borderBottom: '1px solid var(--color-grey-850)' }}>
          {/* 이미지 플레이스홀더 */}
          <div style={{
            width: '101px',
            height: '101px',
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <div className="label1" style={{ color: 'var(--color-white)', fontWeight: 'var(--font-weight-semibold)' }}>이달의 아트 (9월)</div> {/* 폰트 및 색상 조정 */}
            <div className="label1" style={{ color: 'var(--color-white)', fontWeight: 'var(--font-weight-semibold)' }}>34,000원</div> {/* 폰트 및 색상 조정 */}
          </div>
          <ChevronRight size={20} color="var(--color-grey-450)" /> {/* 오른쪽 화살표 아이콘 */}
        </div>
      </div>

      {/* 요청사항 섹션 */}
      <div style={{ padding: '0 20px 32px' }}>
        <h2 className="label1" style={{ color: 'var(--color-white)', marginBottom: '8px' }}>
          요청사항
        </h2>
        <p className="body2" style={{ color: 'var(--color-grey-450)', marginBottom: '16px' }}>
