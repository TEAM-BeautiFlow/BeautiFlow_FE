...
const BookingPage = () => {
  ...
  const [selectedDesigner, setSelectedDesigner] =
    useState("원장님 손하늘 디자이너"); // Figma에 '원장님 손하늘 디자이너' 선택됨으로 초기 설정
  ...
  const afternoonTimes = [
    ...
    "6:30",
    "7:00",
  ]; // Figma에 12:00, 7:30, 8:00, 8:30 없음
  ...
  const generateCalendar = () => {
    ...
    for (let i = 1; i <= totalDaysInMonth; i++) {
      ...
      days.push(
        <div
          key={i}
          className={`label1 cursor-pointer rounded-full py-2.5 text-center text-sm transition-all`}
          style={{
            color: textColor,
    ...
    <div
      ...
      style={{
        ...
        borderColor: isSelected
          ...
          : "var(--color-grey-850)", // 선택 시 Dark Purple 테두리
        backgroundColor: isSelected
          ? "var(--color-purple)"
          : "var(--color-black)", // 선택 시 Primary Purple 배경
      }}
      onClick={onClick}
    >
      <div className="flex items-start space-x-3">
        {" "}
        {/* items-start로 변경하여 이미지와 텍스트 상단 정렬 */}
        {/* 디자이너 이미지 플레이스홀더 및 체크마크 */}
        <div
          className="relative flex h-24 w-24 flex-shrink-0 items-center justify-center overflow-hidden rounded-md" // w-24 h-24, rounded-md (네모)
          style={{
            backgroundColor: isSelected
              ? "var(--color-light-purple)"
              : "var(--color-grey-350)", // 선택 시 보라색 배경
          }}
        >
        ...
        <div className="flex flex-1 flex-col">
          ...
          >
            {" "}
            {/* Font style refined */}
            {description.split("#")[2] && `#${description.split("#")[2]}`}{" "}
            {/* 두 번째 #태그만 표시 */}
          </p>
          <p
            className="caption2"
          ...
          >
            {" "}
            {/* Font style refined */}
            {description.split("#")[3] && description.split("#")[3]}{" "}
            {/* 나머지 설명 */}
          </p>
        </div>
      </div>
