...
const Reservation = (props: ReservationProps) => {
  ...
  const [activeTab, setActiveTab] = useState('시술'); // 현재 활성화된 탭 상태를 관리
  const [selectedCategory, setSelectedCategory] = useState('손'); // 현재 선택된 시술 카테고리
  ...
  const announcements = [
    {
      id: 1,
      title: '공지사항 제목입니다',
      content: '공지 사항을 읽고 예약해주세요. 최대 2줄까지 보입니다. 나머지는 더보기를 통해 확인할 수 있습니다. 디자인은...'
    },
    {
      id: 2,
      title: '휴무일 안내',
      ...
      >
        ...
        <section className="px-5 py-4" style={{ backgroundColor: 'var(--color-black)' }}>
          ...
          <div className="flex justify-between items-center mb-1">
            ...
            {/* '채팅' 버튼만 남김 */}
            <button className="flex items-center gap-1"> {/* '채팅' 아이콘과 텍스트 버튼 */}
              <MessageSquare size={20} color="var(--color-grey-450)" /> {/* Figma에 보이는 색상으로 변경 */}
              <span className="caption1" style={{ color: 'var(--color-grey-450)' }}>채팅</span> {/* Figma에 보이는 폰트와 색상으로 변경 */}
            </button>
          </div>
          {/* 샵 한 줄 소개와 '더보기' 버튼 (Figma에 맞춰 재배치) */}
          {/* '더보기' 버튼을 '말 줄임표...' 옆으로 이동하기 위해 p 태그 내부에 포함 */}
          <p className="body2" style={{ color: 'var(--color-grey-450)' }}>
            네일샵 한 줄 소개입니다. 한 줄 소개는 254px * 42px을 넘어가지 않고, 이후는 말 줄임표...
            <button className="body2 inline-block ml-1" style={{ color: 'var(--color-grey-450)' }}>더보기</button> {/* inline-block으로 인라인 요소처럼 취급, ml-1로 미세 간격 조정 */}
          </p>
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto pb-2 scrollbar-hide">
            ...
            {announcements.map((announcement) => (
              ...
              >
                <h4 className="caption1" style={{ color: 'var(--color-white)', marginBottom: '4px' }}>{announcement.title}</h4>
                <p className="caption2" style={{ color: 'var(--color-grey-450)', lineHeight: '1.5', overflow: 'hidden', display: '-webkit-box', WebkitBoxOrient: 'vertical', WebkitLineClamp: 2 }}> {/* 2줄까지만 보이도록 처리 */}
        ...
        <div className="px-5 py-4 flex-1 overflow-y-auto pb-20" style={{ backgroundColor: 'var(--color-black)' }}>
          ...
          <>
            {/* 카테고리 버튼: 손 / 발 / 기타 */}
            <div className="flex gap-2 mb-6" style={{ backgroundColor: 'var(--color-black)' }}> {/* mb-6으로 간격 조정 */}
              {/* 현재 시술 카테고리 선택 상태가 없으므로, Figma에 맞춰 '손'을 활성화 상태로 가정 */}
              <button className="rounded-full px-2.5 py-1 caption2"
                  style={{
                ...
                <button className="rounded-full px-2.5 py-1 caption2" style={{ backgroundColor: 'var(--color-grey-750)', color: 'var(--color-grey-450)' }}>발</button>
                <button className="rounded-full px-2.5 py-1 caption2" style={{ backgroundColor: 'var(--color-grey-750)', color: 'var(--color-grey-450)' }}>기타</button>
            </div>
