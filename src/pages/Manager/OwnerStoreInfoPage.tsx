...
const OwnerVerificationPage = () => {
  ...
  // '인증완료' 상태로의 전환 로직이 필요합니다.
  // 현재는 예시를 위해 '미인증'으로 설정합니다.
    ...
    try {
      ...
      if (licenseResponse.data.success) {
        if (licenseResponse.data.data) {
          // 이 부분은 백엔드 응답에 따라 '확인중' 또는 '인증완료'로 설정해야 함
  ...
  // 사업자등록증 인증 완료 상태 여부
  const isVerified = businessLicenseStatus === '인증완료';
