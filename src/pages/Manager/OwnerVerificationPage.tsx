...
import { Home, User, MessageSquare, Calendar, MoreHorizontal, ChevronRight, ShieldAlert, Pencil, Image, DollarSign, Clock, Plus } from 'lucide-react';
...
apiClient.interceptors.request.use((config) => {
  const token = 'eyJhbGciOiJIUzI1NiJ9.eyJwcm92aWRlciI6Imtha2FvLXN0YWZmIiwia2FrYW9JZCI6IjQzODc2OTc3OTYiLCJ1c2VySWQiOjYwLCJlbWFpbCI6Impvb245ODA5MjNAbmF2ZXIuY29tIiwiaWF0IjoxNzU1MTQ3NTEyLCJleHAiOjE3NTc3Mzk1MTJ9.usNX4xb-pfiBMM4TPYjlLhmwLeoa2lSFZO6O1KOvLEo';
  if (token) {
...
const OwnerVerificationPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'basic' | 'services' | 'notices'>('basic');
  const [activeServiceCategory, setActiveServiceCategory] = useState<ServiceCategory>('hand');
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      setIsLoading(true);
      try {
        const [shopManageResponse, noticesResponse, hoursResponse, holidaysResponse] = await Promise.allSettled([
        ...
        // --- 매장 정보 처리 ---
        if (shopManageResponse.status === 'fulfilled' && shopManageResponse.value.data?.data) {
        ...
        if (hoursResponse.status === 'fulfilled' && hoursResponse.value.data?.data) {
          ...
          setBusinessHours({ openTime: '', closeTime: '', breakStart: '', breakEnd: '' });
        }
  ...
  // --- 로딩 및 에러 처리 ---
  if (isLoading) {
    return <div className="max-w-sm mx-auto min-h-screen flex items-center justify-center" style={{ backgroundColor: '#1A1A1A', color: 'white' }}>데이터를 불러오는 중...</div>;
  }
  
  if (error) {
    return <div className="max-w-sm mx-auto min-h-screen flex items-center justify-center" style={{ backgroundColor: '#1A1A1A', color: 'white' }}>{error}</div>;
  }

  if (!shopData) {
    return <div className="max-w-sm mx-auto min-h-screen flex items-center justify-center" style={{ backgroundColor: '#1A1A1A', color: 'white' }}>매장 정보를 표시할 수 없습니다.</div>;
  }
  ...
  return (
    ...
    <div className="max-w-sm mx-auto min-h-screen relative text-white font-sans" style={{ backgroundColor: '#1A1A1A' }}>
      <div className='pb-20'>
        <header className="flex items-center justify-between px-5 py-4">
          <span className="text-2xl font-bold" style={{ color: '#8B5CF6' }}>BEAUTIFLOW</span>
        </header>
        
        <div className="px-5 py-4 flex items-center space-x-3">
          <div className="w-14 h-14 rounded-full flex-shrink-0 bg-gray-600">
            {shopData.mainImageUrl && <img src={shopData.mainImageUrl} alt="매장 대표 이미지" className="w-full h-full object-cover rounded-full" />}
          </div>
        ...
        {businessLicenseStatus === '미인증' && (
          <div className="px-5 my-2">
            <button onClick={() => navigate(`/owner-business-registration/${shopId}`)} className="cursor-pointer w-full">
              <div className="flex items-center justify-between w-full p-4 rounded-lg bg-red-900 text-red-300">
        ...
        <div className="flex border-b border-gray-800 px-5 mt-2">
          {[
            { key: 'basic', label: '기본' },
            ...
            <button
              ...
              style={{
                ...
                borderBottomColor: activeTab === tab.key ? '#A78BFA' : 'transparent'
              }}
            >
              {tab.label}
            </button>
        ...
        <div className="px-5 py-4 flex-1 overflow-y-auto">
            ...
            <div className='space-y-6'>
              <div className="p-4 rounded-lg" style={{ backgroundColor: '#1A1A1A' }}>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button className="text-sm" style={{ color: '#A78BFA' }} onClick={() => navigate(`/owner/store-info/${shopId}`)}>수정</button>
                </div>
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-sm leading-relaxed">{shopData.address || '-'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-lg" style={{ backgroundColor: '#1A1A1A' }}>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button className="text-sm" style={{ color: '#A78BFA' }} onClick={() => navigate(`/owner/store-intro/${shopId}`)}>수정</button>
                </div>
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <Pencil size={16} className="text-gray-400" />
                    <span className="text-sm">{shopData.introduction || '-'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Image size={16} className="text-gray-400" />
                    <span className="text-sm">{shopData.mainImageUrl ? '대표 이미지 등록됨' : '-'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-lg" style={{ backgroundColor: '#1A1A1A' }}>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button className="text-sm" style={{ color: '#A78BFA' }} onClick={() => navigate(`/owner/sales/${shopId}`)}>수정</button>
                </div>
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">
                      {(() => {
                        if (shopData.depositAmount === undefined || shopData.depositAmount === null) {
                          return '예약금 미설정';
                        }
              ...
              <div className="p-4 rounded-lg" style={{ backgroundColor: '#1A1A1A' }}>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button className="text-sm" style={{ color: '#A78BFA' }} onClick={() => navigate(`/owner/hours/${shopId}`)}>수정</button>
                </div>
                <div className="space-y-3">
                  <div className="flex items-start gap-2">
                    <Clock size={16} className="text-gray-400 mt-0.5" />
                    <span className="text-sm leading-relaxed">{formatBusinessHours()}</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <Calendar size={16} className="text-gray-400 mt-0.5" />
                    <span className="text-sm leading-relaxed">{formatRegularHoliday()}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'services' && (
             <div>
              <div className="flex gap-2 mb-6">
                {[
                  ...
                  <button 
                    ...
                    style={activeServiceCategory === category.key 
                      ? { backgroundColor: '#6B21A8', borderColor: '#A78BFA' } 
                      : { backgroundColor: 'transparent', borderColor: '#404040' }
                    }
                  >
                    {category.label}
                  </button>
              ...
              {isServiceLoading ? (
                <div className="text-center py-8">시술 목록을 불러오는 중...</div>
              ) : (
                <div className="space-y-4">
                  {services.length === 0 ? (
                    <div className="text-center text-gray-400 py-8">등록된 시술이 없습니다.</div>
                  ) : (
                    services.map(service => (
                      <div key={service.id} className="flex gap-4">
                        <div className="w-20 h-20 rounded-md flex-shrink-0 bg-gray-700">
                           {service.imageUrl && <img src={service.imageUrl} alt={service.name} className="w-full h-full object-cover rounded-md" />}
                        </div>
                        <div className="flex-1">
                          <div className="flex justify-between items-start mb-1">
                            <h4 className="text-base font-medium text-white">{service.name}</h4>
                            <span className="text-xs px-2 py-1 rounded-full bg-gray-800 text-gray-400 flex items-center gap-1">
                              <Clock size={12} />
                              {service.duration}분
                            </span>
                          </div>
                          <p className="text-lg font-bold mb-1" style={{ color: '#A78BFA' }}>{service.price.toLocaleString()}원</p>
          ...
          {activeTab === 'notices' && (
            <div className="space-y-4">
              {notices.length === 0 ? (
                <div className="text-center text-gray-400 py-8">등록된 공지사항이 없습니다.</div>
              ) : (
                notices.map(notice => (
                  <div key={notice.id} className="p-4 rounded-lg" style={{ backgroundColor: '#262626' }}>
                    <div className="flex justify-between items-center mb-2">
                      <h4 className="font-medium text-white">{notice.title}</h4>
                      <ChevronRight size={20} className="text-gray-400" />
                    </div>
                    <p className="text-sm text-gray-400 line-clamp-2 leading-relaxed">{notice.content}</p>
                  </div>
                ))
              )}
            </div>
      ...
      {(activeTab === 'services' || activeTab === 'notices') && (
        <button className="absolute bottom-24 right-5 w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-colors" style={{ backgroundColor: '#8B5CF6' }}>
          <Plus size={28} className="text-white"/>
        </button>
      )}

      <nav className="fixed bottom-0 left-0 right-0 w-full max-w-sm mx-auto flex justify-around items-center py-3 border-t border-gray-800" style={{ backgroundColor: '#1A1A1A' }}>
        ...
        <button className="flex flex-col items-center gap-1" style={{ color: '#A78BFA' }}>
          <Home size={24} />
