...
import {
  putShopTreatments,
  type TreatmentUpsertReq,
} from "../../apis/manager_treatment/treatment";
...
const OwnerEditTreatmentPage = () => {
  ...
  const resolveShopId = async (): Promise<number> => {
    const fromStorage = localStorage.getItem("shopId");
    if (fromStorage) return Number(fromStorage);
    try {
      const user = await getUserInfo();
      // 백엔드에서 userInfo에 shopId를 추가할 예정이라고 하여 안전하게 옵셔널 접근
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const shopId = (user as any)?.shopId ?? (user as any)?.shop?.id;
      if (typeof shopId === "number") return shopId;
    } catch (_) {
      // 무시하고 아래 에러 던짐
    }
    throw new Error("shopId를 찾을 수 없습니다. 로그인 정보를 확인해주세요.");
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const mappedCategory = (categoryMap[category] ??
        category) as TreatmentUpsertReq["category"];

      const payload: TreatmentUpsertReq[] = [
        {
          id: null,
          ...
          optionGroups: [
            {
              ...
              items: options.map(o => ({
                id: null,
                name: o.name,
                extraPrice: Number(o.price) || 0,
                extraMinutes: Number(o.time) || 0,
                description: "",
              })),
            },
          ],
      ...
      const res = await putShopTreatments(shopId, payload);
      console.log("시술 저장 성공", res);
      alert("시술이 저장되었습니다.");
    } catch (e) {
      console.error("시술 저장 실패", e);
      alert("시술 저장에 실패했습니다. 다시 시도해주세요.");
    }
  };
  ...
  function handleOptionChange<K extends keyof TreatmentOption>(
    id: number,
    field: K,
